...
def test_pg_below():
    ...
    assert stack.price_guard.authoritative_check("prod_001", 1.0) == False

@test("PriceGuard allows above-floor price")
def test_pg_above():
    stack = get_guardrail_stack()
    # prod_001 floor is probably around 17000
    assert stack.price_guard.authoritative_check("prod_001", 20000.0) == True

@test("PriceGuard blocks zero price")
def test_pg_zero():
    stack = get_guardrail_stack()
    assert stack.price_guard.authoritative_check("prod_001", 0.0) == False

@test("PriceGuard blocks negative price")
def test_pg_negative():
    stack = get_guardrail_stack()
    assert stack.price_guard.authoritative_check("prod_001", -10.0) == False

@test("PriceGuard check_price_tool never returns floor number")
def test_pg_tool():
    stack = get_guardrail_stack()
    tool_resp = stack.price_guard.check_price("prod_001", 10.0)
...
def test_inv_loaded():
    stack = get_guardrail_stack()
    assert stack.inventory.available("prod_001") >= 0
...
def test_inv_reserve():
    stack = get_guardrail_stack()
    av = stack.inventory.available("prod_001")
    if av > 0:
        res = stack.inventory.reserve("prod_001", "test", 1)
        assert stack.inventory.available("prod_001") == av - 1
...
def test_gate_below():
    stack = get_guardrail_stack()
    items = [CheckoutItem(sku="prod_001", agreed_price=1.0, quantity=1)]
...
def test_gate_above():
    stack = get_guardrail_stack()
    items = [CheckoutItem(sku="prod_001", agreed_price=20000.0, quantity=1)]
